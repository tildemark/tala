...
entries.push({
  id: `je-${je}`,
  journalNumber: `JE-2025-${String(je).padStart(3, '0')}`,
...
// Monthly revenue - varying amounts
const revenueByMonth = [65000, 72000, 68000, 85000, 92000, 88000, 95000, 78000, 82000, 90000, 98000, 105000];
for (let month = 1; month <= 12; month++) {
  entries.push({
    id: `je-${je}`,
    journalNumber: `JE-2025-${String(je).padStart(3, '0')}`,
    entryDate: `2025-${String(month).padStart(2, '0')}-05`,
    ...
    status: 'posted',
    items: [
      { accountId: '1', debit: revenueByMonth[month - 1], credit: 0, description: 'Revenue collection' },
      { accountId: '22', debit: 0, credit: revenueByMonth[month - 1], description: 'Consulting revenue' },
    ],
  });
  je++;
}
...
for (let month = 1; month <= 12; month++) {
  entries.push({
    id: `je-${je}`,
    journalNumber: `JE-2025-${String(je).padStart(3, '0')}`,
    entryDate: `2025-${String(month).padStart(2, '0')}-28`,
    ...
    status: 'posted',
    items: [
      { accountId: '29', debit: 250000, credit: 0, description: 'Staff salaries' },
      { accountId: '1', debit: 0, credit: 250000, description: 'Salary payment' },
    ],
  });
  je++;
}
...
for (let month = 1; month <= 12; month++) {
  entries.push({
    ...
    description: `Monthly rent - ${new Date(2025, month - 1).toLocaleString('en-US', { month: 'long' })}`,
    referenceNo: `EXP-7100-${String(month).padStart(2, '0')}-2025`,
    status: 'posted',
    items: [
      { accountId: '34', debit: 50000, credit: 0, description: 'Monthly rent' },
      { accountId: '1', debit: 0, credit: 50000, description: 'Payment' },
    ],
  });
  je++;
}

// Quarterly subcontractor costs
[1, 4, 7, 10].forEach((month) => {
  entries.push({
    id: `je-${je}`,
    journalNumber: `JE-2025-${String(je).padStart(3, '0')}`,
    entryDate: `2025-${String(month).padStart(2, '0')}-10`,
    description: `Quarterly Subcontractor Costs - Q${Math.ceil(month / 3)}`,
    referenceNo: `SUB-Q${Math.ceil(month / 3)}-2025`,
    status: 'posted',
    items: [
      { accountId: '26', debit: 30000, credit: 0, description: 'Subcontractor payment' },
      { accountId: '1', debit: 0, credit: 30000, description: 'Payment' },
    ],
  });
  je++;
});
...
[1, 4, 7, 10].forEach((month) => {
  entries.push({
    ...
    description: `Quarterly Depreciation - Q${Math.ceil(month / 3)}`,
    referenceNo: `DEP-Q${Math.ceil(month / 3)}-2025`,
    status: 'posted',
    items: [
      { accountId: '42', debit: 5000, credit: 0, description: 'Equipment depreciation' },
      { accountId: '7', debit: 0, credit: 5000, description: 'Accumulated depreciation' },
    ],
  });
  je++;
});

const data = { accounts, entries };

fs.writeFileSync('./scripts/service-firm-clean.json', JSON.stringify(data, null, 2));
console.log(`Generated ${accounts.length} accounts and ${entries.length} journal entries`);
console.log('Saved to scripts/service-firm-clean.json');
