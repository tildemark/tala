...
export default function PurchaseInvoicesPage() {
  ...
  const fetchInvoices = async () => {
    try {
      setLoading(true);
      const response = await fetch('http://localhost:3001/api/invoices?type=purchase');
      if (response.ok) {
        const data = await response.json();
  ...
  const handleMarkPaid = async (id: string) => {
    try {
      const response = await fetch(`http://localhost:3001/api/invoices/${id}/mark-paid`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ paymentMethod: 'check' }),
      });
  ...
  return (
    <div className="space-y-6 bg-slate-50 p-6 rounded-lg">
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border border-slate-200">
        <h1 className="text-4xl font-bold text-tala-primary">Purchase Invoices</h1>
        <p className="text-slate-600 mt-2">Create and manage vendor invoices</p>
      </div>
      ...
      {showForm && (
        <Card className="border-tala-primary/20 bg-white shadow-md">
          <CardHeader className="bg-gradient-to-r from-tala-primary/5 to-tala-secondary/5 border-b border-tala-primary/10">
            <CardTitle className="text-tala-primary">Create New Purchase Invoice</CardTitle>
          </CardHeader>
      ...
      <Card className="border-slate-200 bg-white shadow-md">
        <CardHeader className="bg-gradient-to-r from-tala-primary/5 to-tala-secondary/5 border-b border-tala-primary/10">
          <CardTitle className="text-tala-primary">Purchase Invoices ({filteredInvoices.length})</CardTitle>
        </CardHeader>
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-slate-50 border-b-2 border-tala-primary/20">
                  ...
                  <TableHead className="font-bold text-tala-primary">Vendor</TableHead>
              ...
              <TableBody>
                {filteredInvoices.map((invoice) => (
                  <TableRow key={invoice.id} className="border-b border-slate-200 hover:bg-tala-primary/5">
                    ...
                    <TableCell className="text-slate-700">{getVendorName(invoice.customerId)}</TableCell>
